...
public class ObjLoader {
	...
	public static Figure load(String path) throws IOException
	{
		DataInputStream in = new DataInputStream(
				GLES.cxt.getAssets().open(path));
		...
		while(line != null)
		{
			...
			else if(word[0].equals("f"))
			{
				...
				for(int i=0; i<index.length; i++)
				{
					...
					else{Log.v(MainActivity.Tag,"AN ERROR OCCURED IN PARSE f");}
				}
				indexs.add(index);
			}
	...
	//MTLの読み込み
	public static void loadMtl(HashMap<String, Material> materials, String path) throws IOException
	{
		DataInputStream in = new DataInputStream(GLES.cxt.getAssets().open(path));
		Material material = null;
		materials.clear();
		String line = in.readLine();
		...
		while(line != null)
		{
			...
			{
				material.shininess = new float[]{Float.parseFloat(word[1])};
			}
